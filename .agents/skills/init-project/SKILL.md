...
3. **Create the Folder**: Create a new project directory named `<project-name>/`. If you're unsure where new project folders should live in this workspace, ask the user once — don't guess at a folder-naming convention.

4. **Copy Template**: Copy all files from `templates/design-project/` into the new `<project-name>/` folder. Do NOT copy `templates/pattern-library.md` — it is vault-wide, shared across all projects, and stays where it is.
...
8. **Save Answers**: Once the user answers, update the new project's `FOLDER-INSTRUCTIONS.md` Layer 4 section with their answers.
