...
The known-roster reference for this project. `granola-custom-template.md`'s **👥 Stakeholder Register** section reads from here for name → role/team lookup instead of hardcoding names in the prompt itself. Keep this file current instead of editing the template.
...
- **If someone's role is genuinely ambiguous, leave Function blank rather than guessing.** The Granola template already has a `TBC` fallback for this — don't pre-fill a guess here that then looks authoritative.
...
Note here who has final sign-off, so the Granola template's "有最終核准權的人是否全程在場" check has something to check against.
